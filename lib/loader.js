...
module.exports = function(content, map) {
  ...
  function resolveVueModule(content) {
    ...
    for (let j = 0; j < children.length; j++) {
      switch (children[j].name) {
        case 'template':
          tree.template = markdown
          .render(he.decode(childRenderHTML($(children[j]).html())))
          .replace(
            /\{\{([^\}\}]+)\}\}/g,
            `<span class="token vue-symbol">{{</span>$1<span class="token vue-expression-detal">}}</span>`
          );
          break;
        case 'script':
          ...
          break;
        case 'describe':
          tree.describe = markdown.render($(children[j]).text());
  ...
  function previewHTML(tree, index) {
    return `<preview>
      ...
      <div slot="describe" class="preview-describe">${tree.describe}</div>
  ...
  function scriptHTML(sourceWrapReative, _components) {
    ...
    import Preview from '${sourceWrapReative}';
    ${imports.join('\n')}
  ...
  function codeHTML(template, script) {
    return `<template>
      <div class="preview-container">
        <div class="markdown-container">${template}</div>
      </div>
    </template>
    <script>
      ${script}
